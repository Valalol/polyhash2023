...
def solve(challenge_data):
    ...
    while tick < deadline and order_index < len(orders):
        ...
        for i in range (drone_count):
            ...
            if drone.state == 0 and tick < deadline:
                ...
                for item in order_remaining_items_hist:
                    if selected_warehouse.predicted_products_info[item] > 0:
                        amount = min(selected_warehouse.predicted_products_info[item], order_remaining_items_hist[item])
                ...
                while calculate_weigth(products, products_weight) + products_weight[product] < max_load and item_index <= len(interesting_items) - 1:
                ...
                for product_type in products:
                    for _ in range(products[product_type]):
                        order.remaining_items.remove(product_type)
                        if debug:
                            print(f"Started loading product {product_type} from warehouse {selected_warehouse.warehouse_id} at tick {tick} by drone {i} for order {order.order_id}")
                        solution += f"{i} L {selected_warehouse.warehouse_id} {product_type} 1\n"
                        drone.orders_memory.append([order, product_type])
                drone.state = 1
                ...
                if len(drone.orders_memory) != 0:
                    ...
                    if debug:
                        print(f"Drone {i} Started delivering product {memory_product_type} for order {memory_order.order_id} at tick {tick}")
        ...
        for i in range (drone_count):
            ...
            if drone.state == 1:
                if len(drone.orders_memory) != 0:
                    ...
                    if debug:
                        print(f"Drone {i} Started delivering product {product_type} for order {order.order_id} at tick {tick} for order {order.order_id}")
